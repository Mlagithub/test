...
Subroutine Rotate( i,Tstep,Njc )
	...
	Do while( .True. )
		if( i==25 )then
			print*,i,njc,Pt(i)%X,Xtemp, Tstep
		EndiF
		If( Tstep<0.0 )then
			If( Pt(i)%X<Xtemp ) Exit
			!If( Pt(i)%X<Xtemp .and. Abs(Pt(i)%Y-Pt(Njc)%Y)/Abs(Pt(i)%X-Pt(Njc)%X)<0.7 ) Exit
		Else
			If( Pt(i)%X>Xtemp ) Exit
			!If( Pt(i)%X>Xtemp .and. Abs(Pt(i)%Y-Pt(Njc)%Y)/Abs(Pt(i)%X-Pt(Njc)%X)<0.7 ) Exit
		End If
