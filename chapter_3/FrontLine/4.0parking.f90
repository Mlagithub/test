...
Module Quan_Ju_Bian_Lian
	...
	Real(Kind = Myrealk) :: 				Br, Rinf , Bl, Bt, FillRate, Bb, Accuracy, Accuracyo
...
Subroutine Particle_Get(i)
    ...
    Allocate(Items)
	jj = ForwardLine%Bh
	! If pointer is at right edge, move it to Head
	! If want to particle next to right edge please annotate this part
	If( ForwardLine%Next%Bh == -2 )then
		ForwardLine =>Head
	End If
	! If pointer is at lefg edge, move it to next pointer
	! If want to particle next to left edge please annotate this part
...
Program Open_Form
    ...
    Use Fl
    Implicit None
    Integer :: 					Qk = 1, Hbh, Qbh, Bj, j, Fid, Err, Ntmp=1 
...
!//---------------------------------------------参数设置-----------------------------------------//
	...
	If( Conf=='Y' .or. Conf=='y' )then
		...
		If( Err/=0 )then
			...
			Read(*,*)
		End If
		Read( 11,*, Iostat = Err ) Accuracyo
		If( Err /= 0 .or. Accuracyo>1.0 )then
			Print*,Accuracyo
	...
	Write( 11,"(E10.3,2x,A30)" ) Accuracyo, 'Newton ineration Accuracy'
