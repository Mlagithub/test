...
Real Function DeltaQ( i,j )
	...
	DeltaQ = 0.02*( Pt(j)%Alpha*A2*Pt(j)%Pd*(1.0-Pt(i)%Pd) - Pt(i)%Alpha*A1*Pt(i)%Pd*(1.0-Pt(j)%Pd) )
...
Subroutine GetElectricField()
	Implicit None
	Integer :: 						i, k
	Real( kind=MyRk ):: 			Hei, Edis, Etmp

 	ElectricField(:) = 0.0
	Do i = 1, N_Total
		If( Pt(i)%Y<H_SandBed ) Cycle
		Hei = H_Electric
		k = 0
		Do while( .True. ) 
			...
			If( Edis<1E-5 ) Cycle
			!Etmp = -(Pt(i)%Y-Hei)/Abs(Pt(i)%Y-Hei)*Abs(Pt(i)%Y-Hei)/Sqrt(Edis)*9.0E9*Pt(i)%Q/Edis !// Decomposition electric field to Z direction
			Etmp = -(Pt(i)%Y-Hei)/Abs(Pt(i)%Y-Hei)*9.0E9*Pt(i)%Q/Edis !// No decomposition
			ElectricField(k) = ElectricField(k) + Etmp
		End Do 
	End Do 
	!ElectricField = ElectricField/CellOfElectricField
	Open( 123,File='ElecField.txt',Access='Append' )
	Do i = 1, Size(ElectricField,1)
		Write( 123,'(5(E13.6,2x))' ) Real(i)*CellOfElectricField, ElectricField(i)
	End Do 
	Close( 123 )
	Forall( i=1:Size(ElectricField,1) )
		Esum(i) = Esum(i) + ElectricField(i)
	End Forall
...
Subroutine GetElectricField_1()
	Implicit None
	Integer :: 						i, k
	Real( kind=MyRk ):: 			Hei, Edis, Eps
	Real( kind=MyRk ), Allocatable::Etmp(:)


	Eps = 8.85E-12
	Allocate( Etmp( Size(ElectricField,1) ) )
	Etmp = 0.0
 	ElectricField(:) = 0.0
	Do i = 1, N_Total
		If( Pt(i)%Y<H_SandBed ) Cycle
		k = Floor( (Pt(i)%Y-H_SandBed)/CellOfElectricField ) + 1
		Etmp(k) = Etmp(k) + Pt(i)%Q
	End Do 

	Do i = 1, Size(ElectricField,1)
		ElectricField(i) = Etmp(i)/Length/Eps
		Esum(i) = Esum(i) + ElectricField(i)
	End Do
...
End Subroutine GetElectricField_1
