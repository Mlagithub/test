...
Module Mla
    ...
    Integer :: Mx
    Real(kind=Myrealk), Parameter :: G=9.8, Den=2200, Ym=0.5E9, P=0.4, Pai=3.1415926, K_Spring=0.05E8, Rho=0.012
...
Subroutine Ya_Suo_Lian
    Implicit None    
    Real(Kind=Myrealk) :: X1, X2, Gama12=0.0, Gama_SS=0.0, Fn1=0.0, Fn2=0.0, Lamda, Delta, M1, M2, M, Deltg, Deltaa, Y1, Y2    
    ...
    Do     
        ...
        Delta = Abs(Gama12)    
        If( Gama12<=0.0 )then    
            Deltg= Abs(Gama12)-Gama_SS    
            Deltaa = Deltg/Deltt    
            If( Deltaa<0.0 .And. Flag==.True. )then    
                Deltamax = Abs(Gama_SS)    
                jj = jj + 1    
                Flag = .False.    
            End If    
            Select Case( Mx )    
            Case( 1 )    
	            Fn1 = k12*Abs(Gama12)**1.5	
            Case( 2 )    
                Const = (log(Epsilon))**2                
                Bn = Sqrt((4.0*M1*k_Spring*Const) / (Pai*Pai + Const))    
                Fn1 = k_Spring*Abs(Gama12) + Bn*Deltaa    
            Case( 3 )
        ...
        Fn1 = 0.0  
        Gama_SS = Abs(Gama12)    
        If ( (Gama12>0.0 .And. V1>0.0) .or. (Flag1==.True. .and. Flag==.False.) )then
            Exit 
        End If
    End Do 
...
Subroutine Electric_Calculation(Pai_D)
	...
	Real(Kind = Myrealk), Parameter ::  N = 1.0E18, Q = 1.6E-19, Kb = 1.38E-23, Den = 2.2E3,&
										...
										Mol = 6.02E23, H = 6.63E-34, Alpha = 0.5
    ...
    If(Mx == 3)then
        ...
        If(Deltamax > DeltaXing)then
            ...
            A1 = 2*Pai*R1**2*(1.0-Sqrt(1.0 - R_Equ*DeltaXing/R1**2))
            A2 = 2*Pai*R2**2*(1.0-Sqrt(1.0 - R_Equ*DeltaXing/R2**2))
        Else
    ...
    Select Case( Mx )
    Case( 1 )
        Write(21,"(F13.6,5x,E13.6)")  -V_Impact, -Deltaq    !R1/R2, -Deltaq
    Case( 2 )
        Write(22,"(F13.6,5x,E13.6)")  -V_Impact, -Deltaq    !R1/R2, -Deltaq
    Case( 3 )
        Write(23,"(F13.6,5x,E13.6)")  -V_Impact, -Deltaq    !R1/R2, -Deltaq
    Case( 4 )
        Write(24,"(F13.6,5x,E13.6)")  -V_Impact, -Deltaq    !R1/R2, -Deltaq
    End Select
...
Program Main
    ...
    Open(25, File = '10-5.txt')
    Open(26, File = 'Information.txt',  Access = 'Append')
    ...
    Do Mx = 1, 4 
        ...
        DO while(V_Impact >= -5.9)
            V_Impact = V_Impact - 0.1
            Deltt = 1.0E-7 
            Epsilon = 1.887*Exp(-((-V_Impact+38.2)/42.83)**2)
            Call Ya_Suo_Lian
    ...
    End Do 
    Write(25,*) '4'
    Write(26,"('Rho = ',F10.5)") Rho
    Write(26,"('Epsilon = ',F10.5)") Epsilon
    Write(26,"('R2 = ',F10.5)") R2
    Write(26,*)
    Write(26,*)
    ...
    Call system('"D:\Program Files (x86)\Matlab\R2010b\bin\matlab.exe" -r Article')
